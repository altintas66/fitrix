...
class Abonnement_Vertrag { 
    ...
    */
	...
	public function ubepruefe_ob_vertrag_rechnung_faellig($vertrag)
	{
		...
		if($start > $this->heute) return false;
    ...
    public function get_abrechnungszeitraum($naechste_faelligkeit, $zyklus_anzahl_monate)
    {
        if($naechste_faelligkeit == null) $naechste_faelligkeit = $this->helper->get_english_datetime_now();

        $dt_naechste_faelligkeit = new DateTime($naechste_faelligkeit);        
        $bis = new DateTime($naechste_faelligkeit);        
        $bis = $bis->modify('+'.intval($zyklus_anzahl_monate).' month');

        return array(
            'von' => $dt_naechste_faelligkeit->format('d.m.Y'),
            'bis' => $bis->format('d.m.Y')
        );

    }

    public function get_faellig_tr_class($naechste_faelligkeit)
    {
        if($naechste_faelligkeit == null) return '';
        $dt_naechste_faelligkeit = new DateTime($naechste_faelligkeit);

        if($dt_naechste_faelligkeit <= $this->heute) return 'bg-warning-light';
        else return '';
    }
