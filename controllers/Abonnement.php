...
class Abonnement { 
	...
	public function __construct($db, $helper, $html, $kunde, $rechnung, $rechnung_position, $rechnung_ticket, $abonnement_vertrag, $abonnement_vertrag_rechnung, $einstellungen, $parkwin, $zammad, $api, $email) 
	{
	...
	**/
	...
	public function erstelle_rechnung_aus_vertraegen($abonnement, $vertraege, $has_parkwin = false, $has_zammad = false, $parkwin_artikel_nummer = null, $zammad_artikel_nummer = null)
	{
		...
		if($rechnung_result['result'] == false) return;
		...
		foreach($vertraege AS $vertrag) {
			...
			$position_result = $this->rechnung_position->insert_individuelle_position(array(
				...
				'artikel_zyklus'       => $vertrag['artikel_zyklus'],
				'artikel_einheit'      => $vertrag['artikel_einheit'],
				'artikel_artikel_typ'  => $vertrag['artikel_artikel_einheit']
			));

			$this->abonnement_vertrag->zyklus_hochzaehlen($vertrag); 
			$this->abonnement_vertrag_rechnung->insert($position_result['id'], $rechnung_result['id']);

			if(($has_parkwin == true) && ($vertrag['artikel_nummer'] == $parkwin_artikel_nummer)) {
				if($kunde['parkwin_system_url'] != '') array_push($parkwin_vertraege, $position_result['id']);
			}
			
			if(($has_zammad == true) && ($vertrag['artikel_nummer'] == $zammad_artikel_nummer)) {
		...
		if($has_zammad == true) {
			$this->update_zammad_vertraege($kunde, $zammad_vertraege, $rechnung_result['id']);
		}
	...
	public function update_zammad_vertraege($kunde, $zammad_vertraege, $rechnung_id)
	{
		// var_dump($zammad_vertraege);exit;
		$heute = new DateTime();
		$heute->modify('- 1month');

		foreach($zammad_vertraege AS $zammad_vertrag_id) {
			//var_dump($zammad_vertrag_id);exit;
			...
			var_dump($kunde['zammad_organization_id']);
			// echo "<br><pre>";
			// print_r($entries);
			// echo "</pre>";
			//exit();
			...
			$menge = 0;
			$tickets_html = '<span style="font-size:13px;">';

			foreach($entries AS $ticket) {
				$time_unit        = floatval($ticket['ticket']['time_unit']);
				...
				// var_dump($rechnung_id);
				// var_dump($ticket['ticket']['number']);
				// var_dump($ticket['ticket']['time_unit']);exit;
				...
				$this->rechnung_ticket->insert(array(
					...
					'takt'         => $ticket['ticket']['time_unit']
				));
				$tickets_html .= '#'.$ticket['ticket']['number'].' ('.intval($ticket['ticket']['time_unit']).') Takt, ';
			}
			...
			$this->rechnung_position->update_feld('artikel_menge', $menge, $zammad_vertrag_id, $rechnung_id);
			$this->rechnung_position->update_feld('artikel_beschreibung', $tickets_html, $zammad_vertrag_id, $rechnung_id);
			var_dump($tickets_html);exit;
